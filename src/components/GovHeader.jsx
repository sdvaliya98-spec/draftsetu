...
const { MenuIcon, SettingsIcon } = window;
    ...
    return (
        ...
        >
            <button
                ...
                className="px-2 py-2 text-xs font-black uppercase tracking-wider text-slate-600 hover:text-blue-600 transition-all flex items-center gap-1.5 h-full focus:outline-none border-b-2 border-transparent hover:border-blue-600"
                type="button"
            >
            ...
            <div className="max-w-none mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between items-center h-20">
                    ...
                    <div className="flex items-center gap-3 cursor-pointer select-none" onClick={() => onNavigate('home')}>
                        <div className="h-11 w-11 rounded-xl bg-gradient-to-br from-blue-700 to-sky-500 flex items-center justify-center text-white shadow-md shadow-blue-500/10 hover:scale-105 transition-transform">
                        ...
                        <div>
                            <h1 className="text-lg md:text-xl font-black tracking-tight text-slate-900 flex items-center gap-2 font-outfit">
                                DraftSetu <span className="font-gujarati text-blue-600 text-base md:text-lg font-bold">(ડ્રાફ્ટસેતુ)</span>
                            </h1>
                            <p className="text-[9px] md:text-[10px] font-black text-slate-400 tracking-wider font-sans uppercase">
                    ...
                    <div className="hidden lg:flex items-center space-x-1 h-full lg:ml-8 lg:mr-auto">
                        {menuItems.map((item, idx) => (
                            <NavItem key={item.id || idx} item={item} onNavigate={onNavigate} />
                        ))}
                    </div>

                    {/* RIGHT: User Actions & Simulator */}
                    <div className="hidden md:flex items-center gap-2">
                        {user && user.is_admin === true && (
                            <div className="flex items-center bg-white rounded-full p-1 border border-slate-200 shadow-sm text-[9px] font-semibold uppercase tracking-wider gap-1">
                                <button
                        ...
                        {user && user.is_admin === true && role === 'admin' && (
                            <button
                                ...
                                className="flex items-center gap-1.5 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-white px-3.5 py-2 rounded-xl font-black text-[9px] uppercase tracking-widest shadow-sm transition-all active:scale-95 btn-premium"
                                type="button"
                            >
                        ...
                        {currentUser ? (
                            <div className="flex items-center gap-2">
                                {userCredits !== null && userCredits !== undefined && (
                                    <button
                                        ...
                                        className="text-[9px] bg-blue-50 text-blue-700 hover:bg-blue-100 hover:text-blue-800 border border-blue-200 px-3 py-2 rounded-xl font-black flex items-center gap-1.5 uppercase tracking-wider transition cursor-pointer"
                                        type="button"
                                ...
                                <span className="text-[9px] bg-slate-50 text-slate-700 px-3 py-2 rounded-xl font-black border border-slate-200/60 flex items-center gap-1.5 uppercase tracking-wider select-none">
                                    <span className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></span>
                                    👤 {currentUser}
                                </span>
                                <button
                                    onClick={onLogout}
                                    className="px-3 py-2 bg-rose-50 hover:bg-rose-100 text-rose-600 rounded-xl text-[9px] font-black tracking-widest uppercase transition border border-rose-200/60"
                                    type="button"
                                >
                                    Logout
                                </button>
                            ...
                            <button
                                ...
                                className="px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-[9px] font-black uppercase tracking-widest transition-all shadow-md shadow-blue-600/10 hover:shadow-lg active:scale-95"
                                type="button"
                            >
                                Log In / Register
                            </button>
                    ...
                    {/* Mobile Hamburger Toggle (for smaller screens) */}
                    <div className="lg:hidden flex items-center gap-3">
                        {user && user.is_admin === true && (
                            <button
                                onClick={() => onRoleChange(role === 'admin' ? 'user' : 'admin')}
                                className="text-[9px] font-black bg-slate-50 border border-slate-200 px-2.5 py-1.5 rounded-lg text-slate-600 uppercase"
                                type="button"
                            >
                        ...
                        <button
                            ...
                            className="p-2 rounded-lg hover:bg-slate-100 text-slate-600 transition focus:outline-none border-0 bg-transparent"
